...
function BondDetails({ formik }: { formik: FormikProps<typeof initialOtherDetails> }) {
	...
	// replacing null with ''
	useEffect(() => {
		if (firstTime.current && formik.values.bonds.length > 0) {
			setTimeout(() => {
				formik.setFieldValue(
					'bonds',
					formik.values.bonds.map(v => ({
						...v,
...
export function OtherDetails({ formik, setStep }: FormikInterface) {
	...
	useEffect(() => {
		if (firstTime.current) {
			formik.setFieldValue('bank_account_number', formik.values.bank_account_number ?? '');
			firstTime.current = false;
		}
	}, [formik]);
